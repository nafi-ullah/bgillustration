...
def addLicensePlateImageNewAlgo(model_path, input_image ,transparent_image=None, license_plate=None):
    try:
    # Load the input image
        # types of the arguments
        log_to_json(f"addLicensePlateImage: input_image type: {type(input_image)} transparent_image type: {type(transparent_image)} license_plate type: {type(license_plate)}", current_file_name)
        input_image.seek(0)
        ...
        quadrilateral = np.array([top_left_final, bottom_left_final, bottom_right_final, top_right_final], dtype=np.int32)
        ...
        if fill_image is None:
            append_to_log(f"Error: Unable to read fill image from BytesIO.")
            return input_image
        ...
        # append_to_log(f"Color mean:, np.mean(fill_image.reshape(-1, fill_image.shape[2]), axis=0)")


        # Warp the fill image
        src_points = np.array([[0, 0], [fill_image.shape[1], 0], [fill_image.shape[1], fill_image.shape[0]], [0, fill_image.shape[0]]], dtype=np.float32)
        dst_points = np.array([top_left_final, top_right_final, bottom_right_final, bottom_left_final], dtype=np.float32)
        ...
        # Ensure 4 channels
        if warped_fill_image.shape[2] == 3:
            # Add a dummy alpha channel (fully opaque)
            alpha_channel = np.ones(warped_fill_image.shape[:2], dtype=np.uint8) * 255
        ...
        # save_image_with_timestamp(output_image, 'outputs/licenseplate', 'outputimage.png')
