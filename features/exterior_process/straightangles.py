...
from features.interiorprocess.interiorbg import  remove_background, save_image_to_s3,  remove_background_premium, save_18_19_image_to_s3
...
from features.functionalites.backgroundoperations.reflection import addReflection, addFlipAndMergeReflection
...
from features.functionalites.backgroundoperations.basicbg.basicbgprocess import add_basic_bg ,add_straight_angle_basic_bg
from features.functionalites.imageoperations.basic import resize_image_bytesio, paste_foreground_on_background_bytesio, blur_image_bytesio, resize_1920_image
...
current_file_name = "features/exterior_process/straightangles.py"
licensepalate_model_path = './models/licenseplate/v495images/v495image_best.pt'


def straight_angles(image_url, userid, global_user_setting_data, catalogue_feature_list):
    ...
    try:
        ...
        image_file = BytesIO(response.content)  # 1920 x 1080 er original image er bytesio convert
        ...
        #     notify_error_status(catalogue_id, angle_id, filename, "There is a technical difficulty, please try again later.", current_file_name)
        #     return None


       #-----------------remove bg-------------      
        
        rem_back = remove_background_premium(image_file) # orignal foreground ta extract kore kon coordinate a extract krse seta dibe
        ...
        transparent_image = rem_back["image"] # foreground image suppose 1500 x 1000
        ...
        ) # crop orignal is  1500 x 1000 er image same ase transparent image. transparent image er background shoho version but crop kora
        ...
        image_file_json_org = resize_image_bytesio(cropped_original, "width", 1920 )
        img_byte_arr_org = image_file_json_org['retruned_image']


        # add padding----
        padding_side = 400 if angle_id in ("1", "6") else 150

        detected_vehicle = add_padding_to_image(img_byte_arr, 0, padding_side, padding_side, 0)
        detected_vehicle.seek(0)

        detected_vehicle_org = add_padding_to_image(img_byte_arr_org, 0, padding_side, padding_side, 0)
        detected_vehicle_org.seek(0)
        ...
        detected_vehicle = cropped_original 
        after_detected_car = img_byte_arr
        # after_detected_car image is transparent image
        # detected_vehicle image is the car image with background
       
        if angle_id == "1" or angle_id == "6":
            ...
            if is_licenseplate_blur == True : 
                ...
                print(f"flag 1 processed image type {type(after_detected_car)}")
                if after_detected_car is None:
                    ...
                    print(f"flag 2 processed image type {type(after_detected_car)} and detected type {type(detected_vehicle)}")
            #  reflected_image_path = save_image_with_timestamp(after_bg_image, 'outputs/dumtest/outputdebug/finalimages', 'blurred_license_image.png')
                ...
                after_detected_car = detected_vehicle
                print(f"flag 3 processed image type {type(after_detected_car)}")
        ...
        after_detected_car.seek(0) # padding shoho 1920 width er image transparent
        # detected_vehicle.seek(0) # padding shoho 1920 width er image background shoho
        ...
        #     else:
        #         resized_width = 1700
        #     print(f"flag 4 processed image type {type(after_detected_car)}")
        #     after_detect_json = resize_image_bytesio(after_detected_car, "width", resized_width )
        ...
        is_add_carpolish = False
        if is_add_carpolish == True:
            img_byte_arr = polish_car(after_detected_car)


        #-------------------full crop or blur crop--------------
        print(f"Crop state value --- {set_crop_val}")
        if set_crop_val == 'blur_crop':
            blurcropped_image = paste_foreground_on_background_bytesio(blur_image_bytesio(image_file, set_blur_intesity),img_byte_arr, foreground_coordinates["left"], foreground_coordinates["top"] )
        ...
        if is_add_reflection == True :
            after_ref_image = addFlipAndMergeReflection(after_detected_car, img_byte_arr,angle_id, is_add_reflection)
        ...
        # else:
        #     after_ref_image = addFlipAndMergeReflection(after_detected_car, img_byte_arr,angle_id)
        ...
        #------------adding logo in background wall--------------    
        if set_def_logo is not None and set_logo_position is not None:
            logo_bytesio = get_dynamic_image(set_def_logo, angle_id)
        else:
            logo_bytesio = None

        save_image_with_timestamp( after_ref_image, 'outputs/ind', 'transparent_cropped_bef.png')
        save_image_with_timestamp(detected_vehicle_org, 'outputs/ind', 'org_cropped_bef.png')
        if set_bg_type == 'basic':
            final_output_stream = add_straight_angle_basic_bg(
                ...
                new_image=detected_vehicle_org, 
